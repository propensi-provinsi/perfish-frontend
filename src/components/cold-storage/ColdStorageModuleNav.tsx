...
import ColdStorageApprovalInbox from "@/components/cold-storage/ColdStorageApprovalInbox";
import { approvalInboxConfigForPath } from "@/lib/coldstorage-approval-inbox";
...
export default function ColdStorageModuleNav() {
  ...
  const inboxConfig = approvalInboxConfigForPath(pathname);
  ...
  return (
    <div className="space-y-3">
      ...
      {inboxConfig && (
        <ColdStorageApprovalInbox
          operationTypes={inboxConfig.operationTypes}
          storageKey={inboxConfig.storageKey}
        />
      )}
    </div>
  );
